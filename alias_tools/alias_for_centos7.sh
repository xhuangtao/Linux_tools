...
alias sapp='rpm -qa | grep' #查询相关app是否存在
alias cip='ip addr show |grep -E "inet"|grep -E  "eth0|ens"' #显示所有ip信息
...
alias free='free -m'
alias f=' free -mhwt'
alias fm='sync; echo 3 > /proc/sys/vm/drop_caches'  #  清理内存高占用，释放网页缓存，目录项和索引（To free pagecache, dentries and inodes）
